...
# Display-only relabeling of group_declared's "TD"/"ADHD" values, applied
# via scale labels= so the underlying factor levels/order are untouched.
...
# ICAR has no established clinical cutoff (unlike aq/bdi/ocir), so no shaded
# bands, dashed cutoff line, or cutoff annotation are drawn here - just the
# raw jittered points and the posterior prediction overlay.
p_panel_a <- ggplot(df, aes(x = group_declared, y = icar)) +
  ...
  # aq_regression_analysis's precedent, so the point does not swallow the whisker.
  ...
  labs(x = "Group", y = "ICAR score (0-16, higher = better)") +
