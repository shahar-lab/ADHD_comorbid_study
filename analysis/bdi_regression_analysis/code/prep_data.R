...
# coefficient is ADHD - TD, already on the raw bdi scale.
df <- df_remove_diagnosis_contradiction |>
  ...
  select(subjectid, group_declared, bdi) |>
  # ASSUMED[no criterion given]: dropped rows with missing bdi
  filter(!is.na(bdi))

cat("Rows in regression sample (ADHD/TD, non-missing bdi):", nrow(df), "\n")
cat("Group counts:\n")
