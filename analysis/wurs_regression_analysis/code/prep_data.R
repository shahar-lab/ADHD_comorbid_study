...
# coefficient is ADHD - TD, already on the raw wurs scale.
df <- df_remove_diagnosis_contradiction |>
  ...
  select(subjectid, group_declared, wurs) |>
  # Per spec: drop rows with missing wurs
  filter(!is.na(wurs))

cat("Rows in regression sample (ADHD/TD, non-missing wurs):", nrow(df), "\n")
cat("Group counts:\n")
