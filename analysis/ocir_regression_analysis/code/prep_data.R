...
# coefficient is ADHD - TD, already on the raw ocir scale.
df <- df_remove_diagnosis_contradiction |>
  ...
  select(subjectid, group_declared, ocir) |>
  # ASSUMED[no criterion given]: dropped rows with missing ocir
  filter(!is.na(ocir))

cat("Rows in regression sample (ADHD/TD, non-missing ocir):", nrow(df), "\n")
cat("Group counts:\n")
