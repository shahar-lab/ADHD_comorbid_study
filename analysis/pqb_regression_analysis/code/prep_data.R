...
# coefficient is ADHD - TD, already on the raw pqb scale. pqb is the 21-item
# PQ-B item-endorsement count (theoretical range 0-21); pqb_distress is a
# separate column and is not used here.
df <- df_remove_diagnosis_contradiction |>
  ...
  select(subjectid, group_declared, pqb) |>
  filter(!is.na(pqb))

cat("Rows in regression sample (ADHD/TD, non-missing pqb):", nrow(df), "\n")
cat("Group counts:\n")
