...
code_dir      <- file.path(project_root, "analysis", "pqb_regression_analysis", "code")
artifacts_dir <- file.path(project_root, "analysis", "pqb_regression_analysis", "artifacts")
output_dir    <- file.path(project_root, "analysis", "pqb_regression_analysis", "output")
...
# 2. Model fitting: pqb ~ group_declared, user-approved priors (saves to artifacts/)
...
# 4. Extract posterior draws needed for all plot panels (saves to artifacts/)
...
# 9. Numeric summary of Panel D: median + 90% CI of proportion above cutoff per group (saves to output/)
source(file.path(code_dir, "summarize_cutoff_proportions.R"))

# 10. Assemble composite figure (A left, B/C/D stacked right; saves to output/)
