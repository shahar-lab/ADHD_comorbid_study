...
# ASSUMED[no known instrument-max for PATAS; used observed-range-based scale
# instead]: unlike AQ's fixed 0-50 scale, no documented theoretical max is
# available here, so the y-axis upper limit is set to the next multiple of 5
# above the observed max patas_sum.
y_max <- ceiling(max(df$patas_sum, na.rm = TRUE) / 5) * 5

# No clinical cutoff exists for PATAS in this project, so - unlike the
# aq/bdi/ocir panel A - there is no shaded cutoff band, dashed cutoff line,
# or cutoff annotation here; only the raw jittered points and the posterior
# group-mean prediction overlay.
p_panel_a <- ggplot(df, aes(x = group_declared, y = patas_sum)) +
  ...
  # plus a separate point layer (median), both keyed off x_pos.
  ...
  labs(x = "Group", y = "PATAS score (sum)") +
