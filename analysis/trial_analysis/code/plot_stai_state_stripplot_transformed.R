#### PLOT STAI-STATE STRIP PLOT (TRANSFORMED 0-100 SCALE) ####
...
group_stats_transformed <- df_stai_state_severity |>
  group_by(group_declared) |>
  summarise(mean_val = mean(stai_state_transformed), sd_val = sd(stai_state_transformed), .groups = "drop")

p_stai_state_transformed <- ggplot(df_stai_state_severity, aes(x = group_declared, y = stai_state_transformed, color = group_declared)) +
  ...
  labs(x = "Group", y = "STAI-State score (0-100 rescaled)",
       title = "STAI-State severity by group (transformed scale)") +
  theme_minimal(base_size = 13) +
  theme(panel.grid = element_blank(), legend.position = "bottom")

plot_name <- "stai_state_stripplot_transformed"

ggsave(file.path(output_dir, paste0(plot_name, ".pdf")),
       plot = p_stai_state_transformed, width = 10, height = 8, bg = "white")

ggsave(file.path(output_dir, paste0(plot_name, ".png")),
       plot = p_stai_state_transformed, width = 10, height = 8, dpi = 300, bg = "white")
