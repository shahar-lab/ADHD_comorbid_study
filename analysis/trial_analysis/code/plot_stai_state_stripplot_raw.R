#### PLOT STAI-STATE STRIP PLOT (RAW SCALE) ####

# Same ADHD/TD color convention established in plot_selfreport_dothistograms.R
group_colors <- c(ADHD = "#0072B2", TD = "#D55E00")
cutoff_raw   <- 40

group_stats_raw <- df_stai_state_severity |>
  group_by(group_declared) |>
  summarise(mean_val = mean(stai_state), sd_val = sd(stai_state), .groups = "drop")

p_stai_state_raw <- ggplot(df_stai_state_severity,
                            aes(x = group_declared, y = stai_state, color = group_declared)) +
  ...
  labs(x = "Group", y = "STAI-State score (0-60)",
       title = "STAI-State severity by group (raw scale)") +
  theme_minimal(base_size = 13) +
  theme(panel.grid = element_blank(), legend.position = "bottom")

plot_name <- "stai_state_stripplot_raw"

ggsave(file.path(output_dir, paste0(plot_name, ".pdf")),
       plot = p_stai_state_raw, width = 10, height = 8, bg = "white")

ggsave(file.path(output_dir, paste0(plot_name, ".png")),
       plot = p_stai_state_raw, width = 10, height = 8, dpi = 300, bg = "white")
