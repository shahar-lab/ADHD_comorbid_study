...
# coefficient is ADHD - TD, already on the raw asrs scale.
df <- df_remove_diagnosis_contradiction |>
  ...
  select(subjectid, group_declared, asrs) |>
  # filter(!is.na(asrs)) - per spec, dropping rows with missing asrs
  filter(!is.na(asrs))

cat("Rows in regression sample (ADHD/TD, non-missing asrs):", nrow(df), "\n")
cat("Group counts:\n")
