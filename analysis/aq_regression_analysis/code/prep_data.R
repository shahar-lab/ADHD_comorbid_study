...
# coefficient is ADHD - TD, already on the raw aq scale.
df <- df_remove_diagnosis_contradiction |>
  ...
  # ASSUMED[no criterion given]: dropped rows with missing aq
  filter(!is.na(aq))

cat("Rows in regression sample (ADHD/TD, non-missing aq):", nrow(df), "\n")
cat("Group counts:\n")
